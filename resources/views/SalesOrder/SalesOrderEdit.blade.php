                    ...
                    <div class="col-lg-6">
                        <div class="form-group ">
                            ...
                            <select class="form-control select4" name="edit_customer_by" id="customer_by"
                                ...
                                @if($sales[0]->cust_id == $client->client_id)
                    ...
                    <div class="col-lg-6">
                        <div class="form-group ">
                            ...
                            <select id="id" class="form-control select4" name="edit_booked_by" id="edit_booked_by"
                                ...
                                @if(count($clients) > 0)
                                @foreach($clients as $client)
                                @if($sales[0]->receiver_id == $client->client_id)
                                <option selected value="{{$client->client_id}}">
                                    {{$client->client_name}}
                                </option>
                                @else
                    ...
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control form-control-user SalesOrder_input" id="person_name"
                                ...
                                value="{{$sales[0]->sales_person_name ?? ''}}">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control form-control-user SalesOrder_input"
                                ...
                                value="{{$sales[0]->sales_person_number ?? ''}}">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control form-control-user SalesOrder_input"
                                ...
                                value="{{$sales[0]->sales_receiving_date ?? ''}}">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control form-control-user SalesOrder_input"
                                ...
                                value="{{$sales[0]->sales_delivery_date ?? ''}}">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group">
                ...
                <div class="col-lg-6 mx-auto">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-user SalesOrder_input" id="order_subject"
                            ...
                            value="{{$sales[0]->sales_order_subject ?? ''}}">
                    </div>
                </div>
            </form>
...
$(function() {
    ...
    var tables = $("#Table").DataTable({
        ...
        "serverSide": true,
        ajax: {
            url: "{{route('SalesOrderItem-Show')}}",
            data: {
                sales_order_id: "{{$sales[0]->sales_order_id}}"
            }
        },
        columns: [{
                data: 'item_id',
...
function SalesOrderItemsStore() {
    ...
    $.ajax({
        type: "POST",
        url: "{{route('SalesOrderItem-Store')}}",
        data: formData,
        processData: false,
        contentType: false,
        error: function(jqXHR, textStatus, errorThrown) {
...
function SalesOrderItemsUpdate() {
    ...
    $.ajax({
        type: "POST",
        url: "{{route('SalesOrderItem-Update')}}",
        data: formData,
        processData: false,
        contentType: false,
        error: function(jqXHR, textStatus, errorThrown) {
...
function SalesOrderItemFile(item_id) {
    ...
    // console.log("data");
    $.get("{{route('SalesOrderItemFile-Show')}}", {
        item_id: item_id
    }, function(data) {
        // console.log(data);
        if (data.length > 0) {
            ...
            for (rows of data) {
                ...
                $("#AttachmentBody").append(
                    '<tr class="removeRow"><td style="max-width:400px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"><a target="_blank" download href="{{url("/")}}/public/SalesOrder/Detail/' +
...
function SalesOrderItemFileStore() {
    ...
    $.ajax({
        type: "POST",
        url: "{{route('SalesOrderItemFile-Store')}}",
        data: formData,
        contentType: false,
        processData: false,
        error: function(jqXHR, textStatus, errorThrown) {
...
$(document).ready(function() {
    ...
    $(document).on('click', '.FileRemove', function() {
        ...
        swal({
            ...
            if (willDelete) {
                $.get("{{route('SalesOrderItemFile-Remove')}}", {
...
function SalesOrderItemRemove(item_id) {
    swal({
        ...
        if (willDelete) {
            $.get("{{route('SalesOrderItem-Remove')}}", {
                item_id: item_id
            }, function(data) {
                console.log(data);
                if (data['success'] == true) {
