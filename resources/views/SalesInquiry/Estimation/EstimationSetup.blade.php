            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <?php $backUrl = 'Sales-Inquiry-'.$inquiry[0]->inquiry_id.'-Estimation'; ?>
                        <a href="{{$backUrl}}" style="font-size: 18px;"><i
                        ...
                        <div class="card-body box-profile">
                            ...
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class=" text-capitalize" style="list-style:none;">
                                    <b> Unit </b>
                                    <a class="float-right">{{$items[0]->uom_name}}</a>
                                    <hr>
                                </li>
                                <li class=" text-capitalize" style="list-style:none;">
                                    <b> Quantity </b> <a class="float-right">{{$items[0]->item_quantity ?? 0}}</a>
                                    <hr>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="card-body box-profile">
                            ...
                            <ul class="list-group list-group-unbordered mb-3">
                                ...
                                <li class="" style="list-style:none;">
                                    <b>Item Description</b> <a class="float-right">{{$items[0]->item_description}}</a>
                                    <hr>
                                </li>
                                <li class="" style="list-style:none;">
                                    <b>Note</b> <a
                                        class="float-right">{{$items[0]->item_note}}</a>
                                    <hr>
                                </li>
                            </ul>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="MaterialStoreForm">
                    @csrf
                    <div class="row">
                        <input type="hidden" class="form-control" name="item_id" id="item_id" value="{{$item_id}}">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control material_required" name="order_quantity"
                                    id="quantity" value="{{$items[0]->item_quantity ?? 0}}" readonly="readonly">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control hour_required" name="order_quantity"
                                    id="order_quantity" placeholder="Order Quantity" value="{{$items[0]->item_quantity ?? 0}}" readonly="readonly">
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    var MaterialTable = $("#MaterialTable").DataTable({
        ...
        "serverSide": true,
        ajax: {
            url: "{{Route('Estimation-Material-Show')}}",
            data: {
                item_id: "{{$item_id}}",
                type: "material",
            },
    ...
    $("#estimation_quantity_unit").keyup(function(){
        if($(this).val() != '' && $(this).val() > 0){
            let total_estimated_qty = "{{$items[0]->item_quantity ?? 0}}" * $("#estimation_quantity_unit").val();
...
function EstimationMaterialStore() {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "{{route('Estimation-Material-Store')}}",
...
function EstimationMaterialUpdate() {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "{{route('Estimation-Material-Update')}}",
...
function EstimationMaterialRemove(estimate_id) {
    swal({
        ...
        if (willDelete) {
            $.get("{{route('Estimation-Material-Remove')}}", {
...
$(document).ready(function() {
    ...
    var HourTable = $("#HourTable").DataTable({
        ...
        "serverSide": true,
        ajax: {
            url: "{{Route('Estimation-Hour-Show')}}",
            data: {
                item_id: "{{$item_id}}",
                type: "hour",
            },
    ...
    $("#estimated_requir_work_hour").keyup(function(){
        if($(this).val() != '' && $(this).val() > 0){
            let total_estimated_qty = "{{$items[0]->item_quantity ?? 0}}" * $("#estimated_requir_work_hour").val();
            // console.log("qty "+total_estimated_qty);
...
function EstimationHourStore() {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "{{route('Estimation-Hour-Store')}}",
...
$(function(){
    ...
    $("#edit_estimated_requir_work_hour").keyup(function(){
        if($(this).val() != '' && $(this).val() > 0){
            let total_estimated_qty = "{{$items[0]->item_quantity ?? 0}}" * $("#edit_estimated_requir_work_hour").val();
            // console.log("qty "+total_estimated_qty);
...
function EstimationHourUpdate() {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "{{route('Estimation-Hour-Update')}}",
...
function EstimationHourRemove(estimate_id) {
    swal({
        ...
        if (willDelete) {
            $.get("{{route('Estimation-Hour-Remove')}}", {
                estimate_id: estimate_id
            }, function(data) {
                // console.log(data);
                // return false;
