      ...
      <div class="card shadow">
        <div class="card-body">
          <div class="row no-gutters align-items-center">
            ...
            <div class="col-auto">
              <a href="{{Route('SalesInquiry-Create')}}" class="btn btn-primary">Add
                Inquiry</a>
            </div>
          </div>
        </div>
...
function SalesInquiryFileModal(inquiry_id) {
  ...
  // console.log("data");
  $.get("{{route('SalesInquiry-File-Show')}}", {
    ...
    if (data.length > 0) {
      ...
      for (rows of data) {
        ...
        $("#AttachmentBody").append(
          '<tr class="removeRow"><td style="max-width:400px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"><a target="_blank" download href="{{url("/")}}/public/Sales/' +
...
function SalesInquiryFileStore() {
  ...
  $.ajax({
    ...
    type: "POST",
    url: "{{route('SalesInquiry-File-Store')}}",
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function() {
  ...
  $(document).on('click', '.FileRemove', function() {
    ...
    swal({
      ...
      if (willDelete) {
        $.get("{{route('SalesInquiry-File-Remove')}}", {
