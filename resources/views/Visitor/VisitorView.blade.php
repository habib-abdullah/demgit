    ...
    <div class="col-md-12 mb-2">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">
            <a href="{{route('Visitor-Log')}}" style="font-size: 18px;"><i class="fas fa-arrow-circle-left"></i> Back
            </a>
          </h3>
        </div>
        <div class="row">
            ...
            <div class="card-body box-profile">
              ...
              <ul class="list-group list-group-unbordered mb-3">
                ...
                <li class=" text-capitalize" style="list-style:none;">
                  <b> Visit Time </b>
                  <a class="float-right">{{$visitors[0]->visit_time ?? ''}}</a>
                  <hr>
                </li>
                <li class=" text-capitalize" style="list-style:none;">
                  <b> Customer </b> <a class="float-right">{{$visitors[0]->company_name ?? ''}}</a>
                  <hr>
                </li>
                <li class=" text-capitalize" style="list-style:none;">
                  <b> Visitor Name </b> <a class="float-right">{{$visitors[0]->visitor_name ?? ''}}</a>
                  <hr>
                </li>
                <li class=" text-capitalize" style="list-style:none;">
                  <b> Mobile </b> <a class="float-right">{{$visitors[0]->visitor_mobile ?? ''}}</a>
                  <hr>
                </li>
              </ul>
            </div>
            ...
            <div class="card-body box-profile">
              ...
              <ul class="list-group list-group-unbordered mb-3">
                ...
                <li class=" " style="list-style:none;">
                  <b>Channel</b> <a class="float-right"> {{$visitors[0]->visitor_type ?? ''}} </a>
                  <hr>
                </li>
                <li class="" style="list-style:none;">
                  <b>Attended By</b> <a class="float-right">{{$visitors[0]->first_name ?? ''}}</a>
                  <hr>
                </li>
                <li class="" style="list-style:none;">
                  <b>Purpose</b> <a class="float-right"> {{$visitors[0]->visitor_purpose ?? ''}} </a>
                  <hr>
                </li>
                <li class="" style="list-style:none;">
                  <b>Note</b> <a class="float-right">{{$visitors[0]->visitor_note ?? ''}}</a>
                  <hr>
                </li>
              </ul>
            </div>
                  ...
                  <div class="table-responsive">
                    <table class="table m-0">
                      <tbody>
                        ...
                        @foreach($attachements as $document)
                        <tr>
                          <td class="text-bold">
                            <p href="javascript:void(0)" id="initial">{{$document->attachment_id}}</p>
                          </td>
                          <td class="">
                            :
                          </td>
                          <td> <a target="_blank"
                              href="{{url('/public/Client/ClientLog/')}}/{{$document->attachment_file}}">{{$document->attachment_file}}</a>
                          </td>
                          <td class="">
                            <button type="button" onclick="LogAttachmentRemove('{{$document->attachment_id}}')"
...
function LogAttachmentRemove(attachment_id) {
  swal({
    ...
    if (willDelete) {
      $.get("{{route('Visitor-Log-Attachemnt-Remove')}}", {
